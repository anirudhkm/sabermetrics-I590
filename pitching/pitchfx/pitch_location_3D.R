
library(ggplot2)
library(plotly)

pitch_location <- function(dat, name)
{
  
  for(i in c("R","L"))
  {
    bat.dat <- dat[dat$batter_hand == i,]
    pitching_loc <- with(bat.dat, data.frame(a=x0,b=y0,c=z0,pitch=pitch_type,event=event))
    bat_loc <- with(bat.dat, data.frame(a=px,b=py,c=pz,pitch=pitch_type,event=event))
    trajectory <- rbind(pitching_loc, bat_loc)
    p <- with(trajectory, plot_ly(x=a, y=b, z=c, color=pitch, text=paste("Event",event),type="scatter3d",mode="markers"))
    p <- add_trace(p, x = c(0,0,0), y = c(0,50,1.4), z=c(0,8,8), text = c("Home plate","Pitchers end", "Batters end"), mode = "text", showlegend = F)
    p<-(layout(p,title=paste(name, "- Pitch location at the pitcher and batter end against", i, "handed batter"),xaxis=list(title="xaxis",range=c(-4,4)),yaxis=list(title="Yaxis"),zaxis=list(title="Zaxis")))
    print(p)
  }
}

dat <- read.csv("pitchfx_pitch.csv", header = T)
# load the pitchers data in R

pat <- dat[dat$pitcher == 450212, ]
# load the data for player Pat Neshak
randy <- dat[dat$pitcher == 329092, ]
# load the data for randy choate
pitch_location(pat, "Pat Neshak")
pitch_location(randy, "Randy Choate")